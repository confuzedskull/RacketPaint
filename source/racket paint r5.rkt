...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname |racket paint r5|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #t #t none #f ())))

(require picturing-programs)

;perfected pan mechanism
...
(define-struct scene (image posn posn2))
(define scene_1 (make-scene (list draw-space draw-space 0) (make-posn 250 250) (make-posn2 250 250)))
...
(define (render-scene current new-scene) (make-dot (dot-diameter current)
                                                   ...
                                                   (dot-color current)
                                                    ...
                                                    (make-scene new-scene (scene-posn (dot-scene current)) (scene-posn2 (dot-scene current)))
                                                    (dot-settings current)))

(define (grab-scene current x y) (make-dot (dot-diameter current)
                                                   (dot-opacity current)
                                                   (dot-color current)
                                                    x y
                                                    (dot-scene current)
                                                    (dot-settings current)))
(define (move-scene current x y) (make-dot (dot-diameter current)
                                                   (dot-opacity current)
                                                   (dot-color current)
                                                    (dot-x current) (dot-y current)
                                                    (make-scene (scene-image (dot-scene current))
                                                                (scene-posn (dot-scene current))
                                                                (make-posn2 (+ (- x (dot-x current)) (posn-x (scene-posn (dot-scene current))))
                                                                           (+ (- y (dot-y current)) (posn-y (scene-posn (dot-scene current))))))
                                                    (dot-settings current)))

(define (drop-scene current) (make-dot (dot-diameter current)
                                            (dot-opacity current)
                                            (dot-color current)
                                            (dot-x current)
                                            (dot-y current)
                                            (make-scene (scene-image (dot-scene current))
                                                        (make-posn (posn2-x (scene-posn2 (dot-scene current)))
                                                                   (posn2-y (scene-posn2 (dot-scene current))))
                                                        (scene-posn2 (dot-scene current)))
                                            (dot-settings current)))
...
(define (draw-on-scene current)
  (cond [ (and (string=? (settings-pan (dot-settings current)) "off")
          ...
          (place-image (colored-dot (dot-diameter current)
                            (dot-opacity current)
                            (dot-color current)) (dot-x current) (dot-y current) (first (scene-image (dot-scene current))))]
        
        
        [else (first (scene-image (dot-scene current)))]))
 ...
 (overlay/align "left" "top"
  ...
  (cond [(string=? (settings-help (dot-settings current)) "on")
        ...
        [(string=? (settings-pan (dot-settings current)) "on")
         (place-image (first (scene-image (dot-scene current)))
                      (posn2-x (scene-posn2 (dot-scene current)))
                      (posn2-y (scene-posn2 (dot-scene current)))
                      white-background)]
                                      [else draw-space])
  ...
  (cond [(string=? mouse-event "button-up") 
         (cond [(<= y 50)
             (change-color current (get-pixel-color x y color-palette))] 
               [(and (string=? (settings-pan (dot-settings current)) "on") (> y 50))
                (drop-scene current)]
        [else (change-posn current 475 25)])]
        [(and (string=? mouse-event "button-down") (> y 50))
         (cond [(string=? (settings-pan (dot-settings current)) "on")
                (grab-scene current x y)]
         [else (render-scene current (cons (draw-on-scene (change-posn current x y)) (scene-image (dot-scene current))))])]        
        [(and (string=? mouse-event "drag") (> y 50))
         (cond [(string=? (settings-pan (dot-settings current)) "on")
                (move-scene current x y)]
         [else (render-scene current (cons (draw-on-scene (change-posn current x y)) (scene-image (dot-scene current))))])]
       ; [(and (string=? mouse-event "move") (> y 50) (string=? (settings-pan (dot-settings current)) "on"))
        ;        (move-scene current x y)]
        [else current]))
...
(define (key-handle current key)
 (cond [(string=? key "left") (decrease-diameter current)]
        ...
        [(and (string=? key "u") (image? (first (rest (scene-image (dot-scene current)))))) (render-scene current (rest (scene-image (dot-scene current))))]
        ...
        [(string=? key "p") (if (string=? (settings-pan (dot-settings current)) "off")
                                (make-dot (dot-diameter current) (dot-opacity current) (dot-color current) (dot-x current) (dot-y current) (dot-scene current) (make-settings (settings-help (dot-settings current))
                                                                                                                                                        ...
                                                                                                                                                        (settings-zoom (dot-settings current))
                                                                                                                                                        "on"))
                                (make-dot (dot-diameter current) (dot-opacity current) (dot-color current) (dot-x current) (dot-y current) (make-scene (cons (place-image (first (scene-image (dot-scene current)))
                      (posn2-x (scene-posn2 (dot-scene current)))
                      (posn2-y (scene-posn2 (dot-scene current)))
                      white-background) (scene-image (dot-scene current)))
                                                                                                                                                       (scene-posn (dot-scene current))
                                                                                                                                                       (scene-posn2 (dot-scene current)))
                                          (make-settings (settings-help (dot-settings current))
                                                                                                                                                                              (settings-grid (dot-settings current))
                                                                                                                                                                              (settings-zoom (dot-settings current))
                                                                                                                                                                              "off")))]                                                                                                                                                
        [(string=? key "c") (render-scene current (cons scene-1 (scene-image (dot-scene current))))]
        [(string=? key "s") (save-image (first (scene-image (dot-scene current))) "racket drawing.png")]
        [(string=? key "add") (render-scene current (cons (scale 1.25 (first (scene-image (dot-scene current)))) (scene-image (dot-scene current))))]
        [(string=? key "subtract") (render-scene current (cons (scale 0.75 (first (scene-image (dot-scene current)))) (scene-image (dot-scene current))))]
        [else current]))
