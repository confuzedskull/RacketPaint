...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname |racket paint r4|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #t #t none #f ())))
...
(define-struct scene (image posn))
(define scene_1 (make-scene (list draw-space draw-space 0) (make-posn 250 250)))
...
(define (render-scene current new-scene) (make-dot (dot-diameter current)
                                                   ...
                                                   (dot-color current)
                                                    ...
                                                    (make-scene new-scene (scene-posn (dot-scene current)))
                                                    (dot-settings current)))
...
(define (move-scene current x y) (make-dot (dot-diameter current)
                                                   ...
                                                   (dot-color current)
                                                    ...
                                                    (make-scene (scene-image (dot-scene current))
                                                                (make-posn (+ (- x (dot-x current)) (posn-x (scene-posn (dot-scene current))))
  ...
  (cond [(string=? (settings-help (dot-settings current)) "on")
        ...
        [(string=? (settings-pan (dot-settings current)) "on")
         (place-image (first (scene-image (dot-scene current)))
                      (posn-x (scene-posn (dot-scene current)))
                      (posn-y (scene-posn (dot-scene current)))
                      clear-background)]
                                      [else draw-space])
...
(define (mouse-handle current x y mouse-event)
  (cond [(string=? mouse-event "button-up") 
        ...
        (render-scene current (cons (draw-on-scene (change-posn current x y)) (scene-image (dot-scene current)))) ]
...
(define (key-handle current key)
 (cond [(string=? key "left") (decrease-diameter current)]
        ...
        [(and (string=? key "u") (image? (first (rest (dot-scene current))))) (render-scene current (rest (scene-image (dot-scene current))))]
