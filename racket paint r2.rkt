...
(define-struct settings (help grid zoom))
(define-struct dot (diameter opacity color x y scene settings))

(define current (make-dot 10 150 "black" 475 25 scene_1 (make-settings "on" "off" 1)))
...
(define (key-handle current key)
 (cond [(string=? key "left") (decrease-diameter current)]
        ...
        [(string=? key "g") (if (string=? (settings-grid (dot-settings current)) "on")
